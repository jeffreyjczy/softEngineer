...
export default function IRASHomePage() {
        ...
        return (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginLeft: '5px', marginTop: '10px', width: '63vw', height: '55px', paddingLeft: '20px', alignItems: 'center', fontSize: '1.5rem', border: '1px solid #e7e7e7', borderRadius: '28px', boxShadow: '0px 0px 20px -18px #000000', }}>
                ...
                <div style={{ fontSize: '1rem' }}>
                    IRAS: {item.IRASStatus}
                </div>
                <button onClick={() => { navigate(`/iraspreviewpage?id=${item.personalInfo.code}`) }} style={{ width: '55px', marginRight: '20px', backgroundColor: '#e7e7e7', color: 'black', borderRadius: '8px', border: '2px solid #e7e7e7' }}>View</button>
            </div>
        )
